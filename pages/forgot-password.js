...
const SignInModalLight = ({ onSwap, pillButtons, ...props }) => {
  ...
  const handleResetOtp = async (event) => {
    ...
    const form = event.currentTarget;
      ...
      try {
        const response = await axios.post(`https://proket.in/tab_otp.php?email=${email}&otp=${otp}`);
  ...
  return (
    ...
    <div className=' mx-auto mt-4 mb-4 shadow rounded-4' style={{ border: "1px black", width: "50%", height:"100%"}}>
      ...
      />
        ...
        {!isSent ? (
            <div>
            <p>Enter the email address associated with your account and we'll send you an OTP to verify your account.</p>
...
/>
            ...
            <Button onClick={()=>{handleOtpSubmit}} type='submit' size='lg' variant={`primary ${pillButtons ? 'rounded-pill' : ''} w-100`}>
              Continue
            </Button>
          </Form>
          <a className='mt-2 mt-sm-2 mt-md-3' onClick={handleResetOtp} style={{ cursor: "pointer", display: "block"}}>
  Resend OTP
</a>
